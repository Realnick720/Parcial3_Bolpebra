...
function mostrarEliminarTipo(e, elemento) {
  ...
  $(modal).find(".text-mensaje-modal").html('Esta seguro que desea eliminar el Tipo ' + "'" + elemento.nombre + "'    ?");
...
function guardarTipo(idTipo, elemento) {
  ...
  var tipoDatos = 'JSON';
  solicitudAjax(url, function (response) {
    guardarTipoExitoso(response, elemento);
  }, datos, tipoDatos, tipo);
}
...
function mostrarDatosTipos() {
  ...
  $.each(_datosTipos, function (index, elemento) {
    ...
    fila.append(col(elemento.idtipo).addClass("alinearCentro")); //fila.append(col(elemento.nombre));
    ...
    fila.append(col(AccionColumna(function (e) {
      mostrarEliminarTipo(e, elemento);
...
//# sourceMappingURL=tipo.dev.js.map
